...
- 页面顶部会出现「设置密码」提示，点一下即可创建管理员账号（保存立即生效）

> 想换成别的宿主机目录，把 `"$PWD:/data"` 的左边部分改掉即可，例如 `-v /volume1/docker/share:/data`。
...
用 `-a` 配好账号后，页面不会再出现「设置密码」提示，直接登录即可。账号密码的路径与权限写法见下文「访问控制」。
